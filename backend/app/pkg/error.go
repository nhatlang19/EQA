...
func PanicException_(key string, message string) {
	if strings.Contains(message, "failed to connect") {
...
func PanicException(responseKey constant.ResponseStatus, msg string) {
	if msg != "" {
		PanicException_(responseKey.GetResponseStatus(), msg)
	} else {
...
func PanicHandler(c *gin.Context) {
	if err := recover(); err != nil {
		...
		case
			...
			c.Abort()
		case
			constant.Forbidden.GetResponseStatus():
			c.JSON(http.StatusForbidden, BuildResponse_(key, msg, Null()))
			c.Abort()
		default:
